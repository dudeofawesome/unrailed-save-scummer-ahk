...
WatchDirectory(save_game_dir . "\|.sav", "onSaveDetected", 0x1)
...
onSaveDetected(old_filepath, new_filepath) {
  ...
  if (!old_filepath and !!new_filepath) {
    ...
    if (last_time_save_loaded + 2000 < A_TickCount) {
      ...
      ; TODO: keep a few versions of saves
      FileCopy, %new_filepath%, %backups_dir%\%filename%.%extension%, 0
    }
    ...
    if (FileExist(backups_dir . "\" . filename . "." . extension)) {
      FileCopy, %backups_dir%\%filename%.%extension%, %save_game_dir%\%filename%.%extension%, 0
    }
  }
}
